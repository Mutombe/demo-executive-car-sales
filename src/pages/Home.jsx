...
import {
  ...
  CaretLeft, CaretRight, Car, CurrencyDollar, Handshake, Rocket,
  ShieldCheck, Buildings, CheckCircle,
...
/* ================================================================
   ICON MAP
   ================================================================ */
const iconMap = { Car, CurrencyDollar, Handshake, Rocket, ShieldCheck, Buildings, Star };
  ...
  return (
    <section ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden bg-navy-950">
      ...
      <div className="absolute right-5 sm:right-8 top-1/2 -translate-y-1/2 z-30 flex flex-col gap-3">
        {heroImages.map((_, i) => (
          <button key={i} onClick={() => setCurrentSlide(i)}
            className={`w-[3px] transition-all duration-700 ${i === currentSlide ? 'h-10 bg-gold-500' : 'h-4 bg-white/20 hover:bg-white/40'}`}
      ...
      <motion.div className="relative z-20 flex flex-col justify-center h-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pt-36" style={{ y: textY, opacity }}>
        <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 1, delay: 0.3, ease: 'easeOut' }}
        ...
        <div className="overflow-hidden">
          {['PRESTIGE', 'REDEFINED', 'ON EVERY DRIVE.'].map((line, i) => (
            <motion.div key={line} initial={{ y: '110%' }} animate={{ y: 0 }}
              ...
              <h1 className={`font-heading leading-[0.92] tracking-tight ${
                line === 'REDEFINED' ? 'bg-gradient-to-r from-gold-500 via-gold-400 to-gold-600 bg-clip-text text-transparent italic' : 'text-white'
              }`} style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)', fontWeight: line === 'REDEFINED' ? 700 : 300 }}>
                {line}
              </h1>
            </motion.div>
          ))}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 1.2 }}
          ...
          <p className="text-white/30 text-xs sm:text-sm uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>
            800+ Vehicles Sold &middot; 50+ Premium Brands &middot; Est. 2015
          </p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.4 }}
          className="flex flex-wrap gap-4 mt-10">
          <Link to="/contact" className="group relative inline-flex items-center gap-3 bg-gold-500 text-navy-950 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:bg-gold-400 hover:shadow-xl hover:shadow-gold-500/20"
            style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.ctaPrimary} <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
          </Link>
          <Link to="/projects" className="group inline-flex items-center gap-3 border border-white/20 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:border-gold-500/50 hover:text-gold-400 hover:bg-white/5"
            style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.ctaSecondary}
          </Link>
        </motion.div>
      ...
      <div className="hidden lg:flex absolute right-8 bottom-12 z-20">
        <span className="text-white/10 text-[10px] uppercase tracking-[0.4em]" style={{ writingMode: 'vertical-rl', fontFamily: 'var(--font-sans)' }}>
          Executive Car Sales &mdash; Harare, Zimbabwe
        </span>
      </div>
    </section>
  );
...
function MarqueeTicker() {
  const items = ['MERCEDES-BENZ', 'BMW', 'RANGE ROVER', 'PORSCHE', 'AUDI', 'TOYOTA', 'LEXUS', 'VOLKSWAGEN'];
              ...
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
                ...
                <Link to={`/services#${services?.items?.[i]?.slug || ''}`}
                  ...
                  <div className="absolute top-4 right-5 z-10">
                    <span className="text-gold-500/15 font-heading text-6xl sm:text-7xl italic leading-none">{String(i + 1).padStart(2, '0')}</span>
                  </div>
    ...
    <section ref={containerRef} className="bg-navy-950 py-24 sm:py-32 lg:py-40 overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 mb-12 sm:mb-16">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }}
          ...
          <div>
            <div className="w-12 h-[2px] bg-gold-500 mb-6" />
            <p className="text-gold-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>The Collection</p>
            <h2 className="font-heading text-white leading-[0.92] italic" style={{ fontSize: 'clamp(2.5rem, 6vw, 5rem)' }}>
              Our <span className="text-gold-500">Showroom</span>
            </h2>
          </div>
          <Link to="/projects" className="group text-white/30 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-gold-500 transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>
  ...
  return (
    <section ref={ref} className="relative bg-navy-950 overflow-hidden">
      <div className="absolute inset-0">
        <div className="absolute top-1/2 left-1/4 -translate-y-1/2 w-48 h-48 bg-gold-500/5 blur-3xl" />
        <div className="absolute top-1/2 right-1/4 -translate-y-1/2 w-48 h-48 bg-gold-500/5 blur-3xl" />
      </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-10 sm:gap-16">
          {stats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              <div className="font-heading text-gold-500 leading-none italic"
        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, ease: 'easeOut' }}>
            ...
            <h2 className="font-heading text-white leading-[0.95] italic mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
              The Executive<br /><span className="text-gold-500">Legacy</span>
            </h2>
            <p className="text-white/50 text-sm sm:text-base leading-relaxed mb-6 max-w-lg" style={{ fontFamily: 'var(--font-sans)' }}>
              Founded in 2015 with a singular vision: to bring a world-class automotive buying experience to Zimbabwe. In a market that often lacks transparency, we chose honesty, quality, and client-first service as our founding principles.
            </p>
            <p className="text-white/35 text-sm leading-relaxed max-w-lg" style={{ fontFamily: 'var(--font-sans)' }}>
              Today, we have placed over 800 vehicles with discerning owners across Zimbabwe, Zambia, and Mozambique. Every vehicle is hand-selected, rigorously inspected, and presented to the standards our clients expect.
            </p>
            ...
            <div className="flex gap-10 sm:gap-16">
              ...
              <div>
                <div className="text-gold-500 font-heading text-3xl sm:text-4xl italic leading-none">800+</div>
                <div className="text-white/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Vehicles Sold</div>
              </div>
              <div>
                <div className="text-gold-500 font-heading text-3xl sm:text-4xl italic leading-none">50+</div>
                <div className="text-white/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Premium Brands</div>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.2, ease: 'easeOut' }} className="relative">
            <div className="relative">
              <div className="overflow-hidden">
                <img src="https://images.unsplash.com/photo-1503376780353-7e6692767b70?w=800&q=80" alt="Executive Car Sales showroom"
              ...
              <div className="absolute -bottom-8 -left-6 sm:-left-10 w-[45%] overflow-hidden border-4 border-navy-950 shadow-2xl">
                <img src="https://images.unsplash.com/photo-1549317661-bd32c8ce0571?w=600&q=80" alt="Range Rover in showroom"
...
function WhyChooseUs() {
  ...
  const inView = useInView(ref, { once: true, margin: '-80px' });
  const points = [
    { title: '150-Point Inspection', desc: 'Every vehicle is mechanically and cosmetically vetted to the highest standard before entering our showroom.' },
    { title: 'Transparent Process', desc: 'No hidden charges, no pressure tactics. Just honest advice and fair pricing from consultation to handover.' },
    { title: 'Global Sourcing Network', desc: 'Access to premium vehicles from Japan, UK, South Africa, and the UAE. If it exists, we can find it.' },
    { title: 'After-Sales Excellence', desc: 'Our relationship continues long after the sale. Warranty support, service network, and priority access to new stock.' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }} className="relative">
            <img src="https://images.unsplash.com/photo-1489824904134-891ab64532f1?w=800&q=80" alt="Premium vehicle detail"
              className="w-full aspect-[4/5] object-cover object-center" loading="lazy" />
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.15 }}>
            ...
            <h2 className="font-heading text-white leading-[0.95] italic mb-12" style={{ fontSize: 'clamp(2rem, 4.5vw, 3rem)' }}>
              Why Choose <span className="text-gold-500">Executive</span>
            </h2>
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="text-center">
          ...
          <div className="flex items-center justify-center gap-6 mt-12">
            <button onClick={prev} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-gold-500 hover:border-gold-500/30 transition-colors duration-300" aria-label="Previous">
              <CaretLeft size={16} />
            </button>
            <div className="flex gap-2">
              {homeTestimonials.map((_, i) => (
                <button key={i} onClick={() => setActive(i)}
                  className={`h-[2px] transition-all duration-500 ${i === active ? 'w-10 bg-gold-500' : 'w-3 bg-white/10 hover:bg-white/25'}`}
                  aria-label={`Testimonial ${i + 1}`} />
              ))}
            </div>
            <button onClick={next} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-gold-500 hover:border-gold-500/30 transition-colors duration-300" aria-label="Next">
              <CaretRight size={16} />
            </button>
          </div>
        </motion.div>
...
function CTASection() {
  ...
  return (
    <section ref={ref} className="relative py-28 sm:py-36 lg:py-48 overflow-hidden">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <img src={homeCta.backgroundImage} alt="Executive Car Sales" className="w-full h-[130%] object-cover object-center" loading="lazy" />
      ...
      <div className="relative z-20 max-w-5xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 1 }}>
          ...
          <h2 className="font-heading text-white leading-[0.92] italic mb-8" style={{ fontSize: 'clamp(2.5rem, 8vw, 6rem)' }}>
            YOUR NEXT CHAPTER<br /><span className="text-gold-500">STARTS HERE</span>
          </h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/contact" className="group relative inline-flex items-center gap-3 bg-gold-500 text-navy-950 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:bg-gold-400 hover:shadow-xl hover:shadow-gold-500/25"
              style={{ fontFamily: 'var(--font-sans)' }}>
            ...
            <a href={`https://wa.me/${business.whatsappNumber}?text=${encodeURIComponent(homeCta.whatsappText)}`} target="_blank" rel="noopener noreferrer"
              className="group inline-flex items-center gap-3 border border-emerald-500/40 text-emerald-400 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:bg-emerald-500/10 hover:border-emerald-500/60"
              style={{ fontFamily: 'var(--font-sans)' }}>
